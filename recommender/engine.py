...
for id in xrange(len(cosine_similarities)):
    similar_indices = cosine_similarities[id].argsort()[-1 * K - 1:-1]
    print("%d %s \n\t is most similar to " % (id, discounts[id]))
    for index in similar_indices:
        print("\t%d %s" % (index, discounts[index]))
