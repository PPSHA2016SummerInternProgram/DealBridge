 	  	...
 	  	function appendFavorite(userId, startIndex, limitNum){
 	  		$.getJSON("/api/favorite", {userId:userId, startIndex:startIndex, limitNum:limitNum}, function(result){
 	  			for (i in result) {
 	  				...
 	  				var str = '<tr onclick=location.href="/discount/' + result[i].discountId + '" favorie-id="' + result[i].favoriteId + '" style="background-color:#ffffff"><td width=30px height=120px style="padding:0px 0px 1px 0px;">' + 
