...
public interface DiscountMapper {
	...
	@Results(value = { @Result(property = "discountId", column = "discount_id"),
			@Result(property = "beginTime", column = "begin_time"), @Result(property = "endTime", column = "end_time"),
	...
	@Results(value = { @Result(property = "discountId", column = "discount_id"),
			...
			@Result(property = "endOfUrl", column = "end_of_url"),
			@Result(property = "bankName", column = "bank_name") })
	@Select("SELECT * FROM discount ORDER BY clickrate DESC LIMIT #{limitNumber}")
	List<Discount> selectByClickrate(@Param("limitNumber") int limitNumber);

	@Update("UPDATE discount SET clickrate=clickrate+1 WHERE discount_id = #{discountId}")
	int increaseClickRate(int discountId);
}
